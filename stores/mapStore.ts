...
export const useMapStore = defineStore('map', {
  state: () => ({
    ...
    contextHoverPopup: null as Popup | null,
    identifyRequestId: 0,
  ...
  actions: {
    ...
    async updateContextHoverPopup(lngLat: LngLat, features: MapGeoJSONFeature[]) {
      ...
      if (!this.contextHoverPopup) {
        this.contextHoverPopup = markRaw(new maplibregl.Popup({
          closeButton: false,
          closeOnClick: false,
          className: 'custom-popup',
          offset: [0, -10],
        }));
        this.contextHoverPopup.addTo(this.map as any);
      }

      this.contextHoverPopup.setLngLat(lngLat).setHTML(html);
    },
    ...
    getIdentifyLayerIdsOnMap(layerId: string): string[] {
      ...
      return [...new Set(layersForIdentify.map((layer: any) => String(layer.id)).filter(Boolean))];
    },
